...
function sakura_scripts()
{
    if (akina_option('jsdelivr_cdn_test')) {
        wp_enqueue_script('js_lib', 'https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/js/main/lib.js', array(), akina_option('cookie_version', ''), true);// wp_enqueue_script('js_lib', get_template_directory_uri() . '/cdn/js/lib.js', array(), SAKURA_VERSION . akina_option('cookie_version', ''), true);
    } else {
    ...
    if (akina_option('app_no_jsdelivr_cdn')) {
        wp_enqueue_style('saukra_css', 'https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/css/main/style.min.css', array(), akina_option('cookie_version', ''));// wp_enqueue_style('saukra_css', get_stylesheet_uri(), array(), SAKURA_VERSION);
        wp_enqueue_script('app', 'https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/js/main/sakura-app.min.js', array(), akina_option('cookie_version', ''), true);// wp_enqueue_script('app', get_template_directory_uri() . '/js/sakura-app.js', array(), SAKURA_VERSION, true);
    } else {
			...
			<div class="contents">
				<div class="comment-arrow">
					<div class="main shadow">
						<div class="profile">
							<a href="<?php comment_author_url();?>" target="_blank" rel="nofollow"><?php echo str_replace('src=', 'src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/image/loading/trans.ajax-spinner-preloader.svg" onerror="imgError(this,1)" data-src=', get_avatar($comment->comment_author_email, '80', '', get_comment_author(), array('class' => array('lazyload')))); ?></a>
						</div>
...
function get_author_class($comment_author_email, $user_id)
{
    ...
    {
        echo '<span class="showGrade0" title="Lv0"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/level/level_0.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 6 && $author_count < 10) {
        echo '<span class="showGrade1" title="Lv1"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/level/level_1.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 10 && $author_count < 20) {
        echo '<span class="showGrade2" title="Lv2"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/level/level_2.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 20 && $author_count < 40) {
        echo '<span class="showGrade3" title="Lv3"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/level/level_3.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 40 && $author_count < 80) {
        echo '<span class="showGrade4" title="Lv4"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/level/level_4.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 80 && $author_count < 160) {
        echo '<span class="showGrade5" title="Lv5"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/level/level_5.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    } else if ($author_count >= 160) {
        echo '<span class="showGrade6" title="Lv6"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/level/level_6.svg" style="height: 1.5em; max-height: 1.5em; display: inline-block;"></span>';
    }
    ...
    if (!empty($bookmarks)) {
        ...
        foreach ($bookmarks as $bookmark) {
            ...
            if (empty($bookmark->link_image)) {
                $bookmark->link_image = 'https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/image/error/Transparent_Akkarin.th.jpg';
            }

            $output .= '<li class="link-item"><a class="link-item-inner effect-apollo" href="' . $bookmark->link_url . '" title="' . $bookmark->link_description . '" target="_blank" rel="friend"><img class="lazyload" onerror="imgError(this,1)" data-src="' . $bookmark->link_image . '" src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/image/loading/trans.ajax-spinner-preloader.svg"><span class="sitename">' . $bookmark->link_name . '</span><div class="linkdes">' . $bookmark->link_description . '</div></a></li>';
        }
        $output .= '</ul>';
    }
    return $output;
...
function custom_html()
{
    if (akina_option('login_bg')) {
        $loginbg = 'https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/image/index/background.jpg'; // akina_option('login_bg');
    } else {
        $loginbg = 'https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/image/index/background.jpg';
    }
    echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/css/custom/customLogin.min.css\" />";
    echo '<script type="text/javascript" src="' . get_template_directory_uri() . '/js/login.js"></script>' . "\n";
    echo '<script type="text/javascript">' . "\n";
    echo 'jQuery("body").prepend("<div class=\"loading\"><img src=\"https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/login_loading.gif\" width=\"58\" height=\"10\"></div><div id=\"bg\"><img /></div>");' . "\n";
    ...
    if (($parent_id != '') && ($spam_confirmed != 'spam') && ($admin_notify != '0') && (!$mail_notify)) {
      ...
      box-shadow: 0px 0px 20px 0px rgba(0, 0, 0, 0.18);">
        <header style="overflow: hidden;">
            <img style="width:100%;z-index: 666;" src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/mail/head.jpg">
        </header>
        ...
        <br>
        <h3>您有一条来自<a style="text-decoration: none;color: #FFB6C1 " target="_blank" href="' . home_url() . '/">' . get_option("blogname") . '</a>的回复</h3>
        <br>
      ...
      <div style="text-align: center;">
          <img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/mail/hr.png" alt="hr" style="width:100%;
...
';
        ...
        $message = str_replace("{{", '<img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/bili/hd/ic_emoji_', $message);
...
function comment_picture_support($content)
{
    ...
    $content = str_replace('[img]', '<br><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/image/loading/trans.ajax-spinner-preloader.svg" data-src="', $content);
    $content = str_replace('[/img]', '" class="lazyload comment_inline_img" onerror="imgError(this)"><br>', $content);
    $content = str_replace('[/img^img]', '" class="lazyload comment_inline_img" onerror="imgError(this)"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/image/loading/trans.ajax-spinner-preloader.svg" data-src="', $content);
    return $content;
}
...
{
    return 'https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/smilies/' . $img;
}
...
function push_smilies()
{
    ...
    foreach ($wpsmiliestrans as $k => $v) {
        $Sname = str_replace(":", "", $k);
        $Svalue = $v;
        $return_smiles = $return_smiles . '<span title="' . $Sname . '" onclick="grin(' . "'" . $Sname . "'" . ')"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/smilies/' . $Svalue . '" /></span>';
    }
...
function push_bili_smilies()
{
    ...
    for ($i = 0; $i < count($name); $i++) {
        ...
        // 选择面版
        $return_smiles = $return_smiles . '<span class="emotion-secter emotion-item emotion-select-parent" onclick="grin(' . "'" . $name[$i] . "'" . ',type = \'Math\')" style="background-image: url(https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/bili/hd/ic_emoji_' . $name[$i] . '.png);"><div class="img emotion-select-child" style="background-image: url(https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/bili/' . $name[$i] . '.png);
        ...
        "></div></span>';
        // 正文转换
        $bilismiliestrans['{{' . $name[$i] . '}}'] = '<span class="emotion-inline emotion-item"><img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/bili/' . $name[$i] . '.png" class="img" style="/*background-image: url();*/
        animation-duration: ' . ($img_height / 32 * 40) . 'ms;
        animation-timing-function: steps(' . ($img_height / 32) . ');
        transform: translateY(-' . ($img_height - 32) . 'px);
        height: ' . $img_height . 'px;
...
{
    $content = str_replace("{{", '<img src="https://cdn.jsdelivr.net/gh/fz6m/Private-web@2.0.0/php/image/bili/hd/ic_emoji_', $content);
...
add_action('pre_get_posts', function ($q) {
    if ( $q->is_home() && $q->is_main_query() ){$q->set( 'posts_per_page', 10 - sizeof(get_option( 'sticky_posts' )) );
        if ( $q->get( 'paged' ) > 1 ) $q->set( 'post__not_in', get_option( 'sticky_posts' ) );
    }
...
function old_content_message($content) {
    if(get_post_type() == 'shuoshuo' || !is_single()) {
        return $content;
    }
	...
	if ($diffTime > 180) {
	$content = '<div class="warn-custom" style="margin-top: 20px;"> 本文最后更新于'.get_the_modified_time(' Y 年 n 月 j 日').
	'，文章已超过 6 个月！内容可能已失效，请自行测试 ~ </div>'.$content;
    }
    $content = '<div class="warn-custom">若图片加载失败，请开启代理或配置 <a href="/375">Github Host</a>，还可帮助今后更好的使用 Github 哦</div>'.$content;
    return $content;
}
